...
> **Estado de verificación por plataforma:** `install.ps1` (Windows) **se corrió de punta a
> punta en Windows real** (`uv tool install`/`upgrade` + `maia install` completo) — confirmado.
> `install.sh` está escrito para ser portable (bash 3.2+, solo POSIX) y su lógica de permisos
> se probó empíricamente en macOS, pero el flujo completo (macOS y Linux) todavía no se corrió
> de punta a punta contra el feed real. Si lo corres, avisa en el repo principal de MAIA (issue
> o mensaje al equipo de IA Practice) para poder quitar este aviso.
